...
if __name__ == "__main__":
  case = "ResNetODE_complex_backtrack100_" + str(args.seed)
  wandb_upload = True
  if wandb_upload:
    import wandb
  ...
  while err_total > tol and it <= maxit:
    ...
    while not refine:
      ...
      if ep >= len(loss_hist) - 1:
        # loss has not improved in the window
        # restore best params and refine
        if jnp.argmin(loss_hist) == 0:
          params = best_params
          opt_state = best_tx
          refine = True
      if (ep+ep_total) % 1000 == 999 or ep == 0 or refine:
        # solve
